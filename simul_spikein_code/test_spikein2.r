...
#Code for testing other methods such as wilcoxon, regression/anova and kolmogorov smirnov
...
r=0.1; ndef=150;#only used if testing imbalance
multiple=100;
...
ns=c(150,300,450,600)#was 200 instead of 150
...
for (u in 1:length(var)){
  ...
  for (k in 1:multiple){
    ...
    #changed my method to k.s
    #esa_p=esa_m;
    esa_p=apply(finalexpr,1,function(e){e=invtrans(e);ks.test(e[indcase],e[indctrl])$p.value});
    fdr_esa=p.adjust(esa_p, method="fdr");
    
    #logistic regression Instead of limma
    ...
    getpval_glm=function(e,f){res=glm(y ~  .,family=binomial(link="logit"), data=data.frame(y=f,x=e)); return(summary(res)$coefficients[2,4])}
    pval_reg=apply(finalexpr,1,function(e){e=invtrans(e);getpval_glm(e,finalpheno)});
    results2=data.frame(1,1,1,pval_reg,p.adjust(pval_reg, method="fdr"))
    
    #Levene test
    levres=apply(finalexpr,1,function(x){x=invtrans(x);leveneTest(x,group=as.factor(finalpheno))$"Pr(>F)"[1]})
    fdr_lev=p.adjust(levres, method="fdr");
    
    #Anova instead of combination test
    #wf=-2*(log(pval_t)+log(levres));
    #combres=1-pchisq(wf,df=4);
    #fdr_comb=p.adjust(combres, method="fdr");
    getpval_ano=function(e,f){res=aov(x ~  y, data=data.frame(y=f,x=e)); return(summary(res)[[1]][1,5])}
    combres=apply(finalexpr,1,function(e){e=invtrans(e);getpval_glm(e,finalpheno)});
    fdr_comb=p.adjust(combres, method="fdr");
...
save.image(paste(resname,"_other.RData",sep=""))
