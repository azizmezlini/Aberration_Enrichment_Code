...
#Code for testing the unidirectional version of our test and assess whether we can model the test statistic under the null
...
local=FALSE;
if(local){
  dir="Downloads/testing_expression/";
  ...
  multiple=100
  k_ind=1;testing_adding_ncase=0;
}

print(paste("Parameters:",r,testing_imbalance_ctrl,testing_imbalance_case));
library(limma)
library(car)#levene
source(paste(dir,"Rproject/functions.r",sep=""))#source("Downloads/testing_expression/Rproject/functions.r")
...
for (u in 1:length(var)){
  ...
  ptm= proc.time();
  calibration=calibrate_test(finalpheno,w0,rep=nrep,doall=TRUE,flatten=flatten,unidirectional=1,ignoremax=20);
  print(proc.time()-ptm)
  
  #y=1-(1:length(calibration))/length(calibration);
  #ind=round(length(calibration)*0.95):length(calibration);ind=sample(ind,10000)
  #x=calibration[ind];x[x<0]=0;y=y[ind];testData=log(1+x)
  #xx <- seq(1,10, length=500)
  #plot(x,-log(y),pch=19,ylim=c(0,15));lines(xx, predict(fit1, data.frame(x=xx)), col="red")
  #fit.norm=fitdistr(calibration,"normal")
  #fit.lognorm <- fitdist(testData, "truncnorm",fix.arg=list(a=0),start = list(mean = mean(testData), sd = sd(testData)),optim.method="L-BFGS-B", lower=c(0, 0))
  
  y=1-(1:length(calibration))/length(calibration);hist(calibration,breaks=1000)
  library(fitdistrplus);descdist(calibration, discrete=FALSE, boot=5000)
  z=(calibration-mean(calibration))/sd(calibration)
  plot(-log(y),-log(pnorm(-z))); lines(c(0,14),c(0,14));
  ks.test(x=z,y='pnorm',alternative='two.sided')
  shapiro.test(calibration[sample(1:length(calibration),5000)])
  ...
  for (k in 1:multiple){
    ...
    #Ancova/anova/lm exploration
    pval_glm=apply(finalexpr,1,function(e)coef(summary(glm(finalpheno~e,family=binomial(link="logit"),data=data.frame(finalpheno=finalpheno,e=e))))[2,4])
    pval_ano=apply(finalexpr,1,function(e)Anova(aov(finalpheno~e,data=data.frame(finalpheno=finalpheno,e=e)), type="III")$`Pr(>F)`[2])
    #dataname=data.frame(finalpheno=finalpheno,e=finalexpr[truegenes[1],]);
    #contrasts(dataname$factorvariable)=contr.poly(3)##( # of levels, i.e. 3) 
    #model.1=aov(finalpheno~e, data=dataname)
    ...
    for (i in 1:nrow(finalexpr)){e=finalexpr[i,];res_esa[[i]]<- aziz.test(finalpheno,e,w0,rep=0,flatten=flatten,unidirectional=1);}
